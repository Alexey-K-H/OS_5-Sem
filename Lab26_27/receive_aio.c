...
void receiving_routine(int socket_fd){
    ...
    char user_input_key;
    struct aiocb receive_request;
    struct aiocb write_request;
    struct aiocb user_input_request;
    ...
    while (!is_socketfd_eof || buffer_bytes_count){
        ...
        if(bytes_to_write){
            write_request = create_aiorq(STDOUT_FILENO, buffer, bytes_to_write);
            requests[1] = &write_request;
            aio_write(&write_request);
        }
        if(!lines_left){
            user_input_request = create_aiorq(STDIN_FILENO, &user_input_key, 1);
