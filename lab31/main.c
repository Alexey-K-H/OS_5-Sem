...
//списочек из сокетов для обхода
struct Queue {
    int size;
...
void dropConnection(int id, char *reason, int server, struct connection *connections, int *connectionsCount, int threadId) {
...
//    write(1, "\n", 1);
    ...
    while (1) {
        ...
        pthread_mutex_lock(&connectionsMutex);
        if (allConnectionsCount / threadsCount >= connectionsCount && queue->size > 0) {

            newClientSocket = getSocket();
        ...
        while (connectionsCount == 0 && queue->size == 0) {

            pthread_cond_wait(&queue->condVar, &queue->checkSizeMutex);
        ...
        if (newClientSocket != -1) {
            connections[connectionsCount - 1].clientSocket = newClientSocket;
            connections[connectionsCount - 1].buffer_size = 0;
            connections[connectionsCount - 1].buffer = NULL;
            connections[connectionsCount - 1].cacheIndex = -1;
            connections[connectionsCount - 1].serverSocket = -1;
            connections[connectionsCount - 1].status = GETTING_REQUEST;
            connections[connectionsCount - 1].id = rand() % 9000 + 1000;
        }

        struct pollfd fds[2 * MAX_CONNECTIONS];
        formPollFds(fds, connectionsCount, connections);

        int polled = poll(fds, connectionsCount * 2, -1);
            ...
            switch (connections[i].status) {
                case GETTING_REQUEST:
                    if (fds[i * 2].revents & POLLHUP) {
                        dropConnection(i, "closed by client", 0, connections, &connectionsCount, threadId);
                    } else if (fds[i * 2].revents & POLLIN) {
                        printf("00\n");
                        ssize_t readCount = recv(connections[i].clientSocket, buf, BUFFER_SIZE, 0);

                        if (readCount <= 0) {
                            dropConnection(i, "connection broken", 0, connections, &connectionsCount, threadId);
                            break;
                        }
                        printf("01\n");
  ...
  //                          printf("WRITING REQUEST CUTBODY()\n");
                            ...
                            if (send(connections[i].serverSocket, connections[i].buffer, connections[i].buffer_size, 0) < 0) {
                                makeCacheInvalid(i);
                                wakeUp(connections, connectionsCount);
                                dropConnection(i, "Failed to write request", 1, connections, &connectionsCount, threadId);
                                break;
                            }
  ...
  //                          printf("FORWARDING REQUEST CUTBODY()\n");
                            ...
                            connections[i].buffer = allocateMemory(connections[i].buffer, &connections[i].buffer_size, readCount);
   ...
   //                         printf("FORWARDING REQUEST\n");
                            ...
                            if (send(connections[i].serverSocket, buf, (size_t) readCount, 0) < 0) {
                                dropConnection(i, "Failed to forwarding response", 1, connections, &connectionsCount, threadId);
                            }
                        } else {
                            dropConnection(i, "Failed to getSocket request from client", 1, connections,
                                           &connectionsCount, threadId);
                        }
                    }
                    break;
 ...
 //                       printf("FORWARDING RESPONSE\n");
                        ...
                        if (readCount < 0) {
                            dropConnection(i, "connection broken", 1, connections, &connectionsCount, threadId);
                            break;
                        }
                        ...
                        if (readCount == 0) {
                            ...
                            dropConnection(i, "Transmission over, socket closed", 1, connections, &connectionsCount, threadId);
                            break;
                        }
                        ...
                        if (send(connections[i].clientSocket, buf, (size_t) readCount, 0) < 0) {
                            makeCacheInvalid(connections[i].cacheIndex);
                            wakeUp(connections, connectionsCount);
                            dropConnection(i, "Client closed connection", 1, connections, &connectionsCount, threadId);
                            break;
                        }
                                    ...
                                    if (statusCode != 200) {
                                        ...
                                        if (cache[connections[i].cacheIndex].alreadyWritten ==
                                            ...
                                            dropConnection(i, "Transmission over, data transfered succesfully", 1,
                                                           connections, &connectionsCount, threadId);
                                        }
                                    }
                                    freeBuffer(&connections[i]);
                                } else {
                ...
                case RESPONDING_FROM_CACHE:
                    if (fds[i * 2].revents & POLLOUT) {
                        ...
                        if (cache[connections[i].cacheIndex].status == INVALID) {
                            pthread_mutex_unlock(&cache[connections[i].cacheIndex].mutex);
                            dropConnection(i, "Failed to load from cache", 0, connections, &connectionsCount, threadId);
                            break;
                        }

                        if ((cache[connections[i].cacheIndex].alreadyWritten == connections[i].cacheBytesWritten ||
                            cache[connections[i].cacheIndex].allSize == 0) && cache[connections[i].cacheIndex].writerId == threadId) {
                        ...
                        if (cache[connections[i].cacheIndex].status != VALID) {
                            if (cache[connections[i].cacheIndex].status == DOWNLOADING) {
                                pthread_mutex_lock(&cache[connections[i].cacheIndex].mutex);
                                while (cache[connections[i].cacheIndex].alreadyWritten == connections[i].cacheBytesWritten ||
                                    ...
                                    pthread_cond_wait(&cache[connections[i].cacheIndex].condVar, &cache[connections[i].cacheIndex].mutex);
                                }
                                pthread_mutex_unlock(&cache[connections[i].cacheIndex].mutex);
                            }
                        ...
                        if (bytesWritten < 0) {
                            dropConnection(i, "connection broken by client", 0, connections, &connectionsCount, threadId);
                            break;
                        }
...
void initThread(pthread_t *thread, int i) {
    if (pthread_create(thread, NULL, work, &threadsId[i]) < 0) {
...
int main(int argc, const char *argv[]) {
    ...
    //квеве(куеуе) - создание очереди для тредов (для последующего обхода)
    queue = createQueue();
